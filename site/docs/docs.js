/* Hydra docs — shared chrome behavior
...
(function(){
  ...
  function applyTheme(theme){
    ...
    try{localStorage.setItem('hydra_theme',theme)}catch(e){}
  }
  function readTheme(){
    try{
      const s=localStorage.getItem('hydra_theme');
      if(s==='light'||s==='dark') return s;
      const old=localStorage.getItem('hydra-theme');
  ...
  const T={
    ...
    'ftr.copy':{zh:'© 2026 Hydra · MIT',en:'© 2026 Hydra · MIT'},
    ...
    // index page hero
    'hero.eyebrow':{zh:'Hydra · 开源终端 AI 编码助手',en:'Hydra · Open-source Terminal AI Coding Assistant'},
  ...
  function detectLang(){
    ...
    if(m) return m[1];
    try{const s=localStorage.getItem('hydra_lang');if(s==='zh'||s==='en') return s;}catch(e){}
    return 'zh';
  }
  let LANG=detectLang();
  try{localStorage.setItem('hydra_lang',LANG)}catch(e){}
