...
- 支持 `WebviewPanel` 编辑器标签页模式（`Hydra: Open in New Tab`）

**Daemon MCP 集成（1 commit）：**
- `AppState` 新增 `Arc<RwLock<Arc<McpRegistry>>>`，启动时后台连接 `~/.hydra/mcp.json` 中配置的 MCP server
...
```bash
# 1. 编译 daemon（Rust 后端）
cd /Users/liguanchen/Desktop/hydra
cargo build -p hydra-daemon --release
...
nohup ./target/release/hydra-daemon > /tmp/hydra-daemon.log 2>&1 &

# 验证 daemon 运行状态
curl http://127.0.0.1:13456/health
# 期望输出: {"status":"ok","version":"4.20.4","service":"hydra-daemon"}
...
3. 在新窗口中：
   - 侧边栏点击 Hydra 图标 → 打开侧边栏聊天
   - Cmd+Shift+P → "Hydra: Open in New Tab" → 在编辑器标签页中打开
   - Cmd+Shift+P → "Hydra: New Conversation" → 新建对话
```
...
# 打包为 .vsix 文件
vsce package
# 输出: hydra-vscode-0.1.0.vsix

# 本地安装测试
code --install-extension hydra-vscode-0.1.0.vsix

# 卸载
code --uninstall-extension atomgit.hydra-vscode
```
...
| 发送消息无响应 | 查看 daemon 日志：`tail -f /tmp/hydra-daemon.log` |
...
| daemon 端口被占用 | `lsof -i :13456` 查看占用进程，`pkill -f hydra-daemon` 停止旧进程 |
