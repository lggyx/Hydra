...
Hydra uses a unified permission model to control access to files, directories, and shell commands.
...
Hydra's permission model is designed to:
...
Hydra currently models external path access with three actions:
...
Hydra also treats the following as sensitive:
...
This gives Hydra a single path policy for file tools instead of per-tool ad hoc logic.
...
For common shell file commands, Hydra extracts path arguments and maps them onto the same shared path approval model used by file tools.
...
Hydra currently does not perform semantic inspection of interpreter code passed through shell commands.
...
Without it, Hydra would need to:
...
Hydra currently uses a unified path-based approval model for file tools and common shell file commands.
