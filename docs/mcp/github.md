...
本文说明如何在 Hydra 中配置 GitHub Remote MCP，并通过 OAuth 登录后使用 GitHub MCP tools。
...
3. 填写：
   - `Application name`：例如 `Hydra MCP Local`
   - `Homepage URL`：`http://127.0.0.1`
   - `Authorization callback URL`：`http://127.0.0.1/callback`
4. 创建完成后，复制 `Client ID`。
5. 生成并复制 `Client secret`。

> Hydra 登录时会使用本地随机端口，例如 `http://127.0.0.1:<random-port>/callback`。GitHub 对 loopback redirect URL 支持随机端口，OAuth App 中 callback 写 `http://127.0.0.1/callback` 即可。
...
```bash
export HYDRA_GITHUB_MCP_CLIENT_ID="<your-github-oauth-client-id>"
...
```bash
hydra mcp add-github-oauth github
```
...
也可以手写 `.mcp.json` 或 `~/.hydra/mcp.json`：
...
```bash
hydra mcp login github --client-id "$HYDRA_GITHUB_MCP_CLIENT_ID"
```
...
4. Hydra 保存 token 到本机配置目录下的 `mcp_auth.toml`。

可以检查 token 是否保存：

```bash
cat ~/.hydra/mcp_auth.toml
```
...
首次连接时 GitHub MCP 的 `tools/list` 可能较慢。如果设置了 `timeout_ms: 60000`，Hydra 的外层 tools/list 超时会按该配置加少量余量等待。
...
当模型调用 MCP 工具时，Hydra 会展示工具审批。确认参数无误后批准执行。
...
```bash
echo "$HYDRA_GITHUB_MCP_CLIENT_ID"
...
项目级 `.mcp.json` 会覆盖同名的全局 `~/.hydra/mcp.json` server。

## 9. 安全注意事项

- 不要提交 GitHub OAuth `Client secret`。
- 不要提交 `~/.hydra/mcp_auth.toml`。
