# Hydra Architecture — Module Map

## Crate Overview

| Crate | Role |
|-------|------|
| **hydra-core** | Agent 引擎 — 工具、上下文、语义分析、LLM 通信 |
| **hydra-tuix** | 终端 UI — retained-mode 渲染、input、modal 选择器、命令分发 |
| **hydra-cli** | CLI 入口 — 参数解析、OAuth 登录 |
| **hydra-daemon** | 后台服务 — HTTP API 服务（独立部署） |
...
| `agent/knowledge.rs` | 179 | **跨 session 知识持久化** — 从 tool result 自动提取端口/密码，"记住这个"手动记录，存入 .hydra/knowledge.md |
...
| `tool/use_skill.rs` | ~80 | **技能调用** — 加载 .hydra/skills/ 下的 markdown 技能 |
| `tool/cd.rs` | ~50 | **切换目录** |
...
| `turn/log.rs` | ~160 | **LLM 请求日志** — 每次 LLM 调用写 JSON 到 ~/.hydra/logs/ |
