...
Hydra 是一款集成在 VS Code 中的开发辅助工具，用于帮助开发者更高效地理解、编辑与管理代码。
...
Hydra 提供面向开发流程的辅助能力，包括代码理解、上下文分析与编辑建议生成，使开发者能够在编辑器中更高效地完成日常开发任务。所有能力均在用户控制下运行，不改变 VS Code 的默认行为模式。
...
1. 安装扩展并打开 Activity Bar 中的 Hydra 面板
2. 首次使用选择模型配置方式：
...
| 命令 | 说明 |
|------|------|
| `Hydra: Open in Side Bar` | 侧边栏打开 |
| `Hydra: Open in New Tab` | 新标签页打开 |
| `Hydra: New Conversation` | 新建会话 |
| `Hydra: Explain Selection` | 解释选中代码 |
| `Hydra: Fix Selection` | 提供修复建议 |
| `Hydra: Optimize Selection` | 提供优化建议 |
| `Hydra: Stop Generation` | 停止当前操作 |
...
| 设置项 | 默认值 | 说明 |
|--------|--------|------|
| `hydra.daemon.port` | `13456` | 本地服务端口 |
| `hydra.daemon.autoStart` | `true` | 是否启用本地服务 |
| `hydra.daemon.binaryPath` | — | 自定义服务路径 |
| `hydra.preferredLocation` | `sidebar` | 默认打开位置 |
| `hydra.autoSave` | `true` | 操作前自动保存 |
| `hydra.sendWithCtrlEnter` | `false` | Ctrl+Enter 发送 |
| `hydra.fontSize` | `13` | 聊天面板字号 |
| `hydra.showInlineHints` | `true` | 显示内联 diff 提示 |

---

## 运行机制说明

Hydra 通过本地运行的辅助服务与 VS Code 进行通信，用于处理上下文、会话和模型请求。

- 所有操作均由用户触发
- 扩展不会在后台执行未授权行为
- 用户发送消息、使用选区命令或附加文件时，相关输入、选区或文件内容会进入请求上下文
- 本地服务会根据用户配置调用对应的模型服务提供方
- 本地服务仅用于提升交互体验，随扩展自动启动，也可通过 `hydra daemon` 手动运行
...
- Hydra 不会向未配置或未授权的第三方服务主动发送用户代码内容
...
- [官网](https://hydra.atomgit.com/)
- [源码仓库](https://atomgit.com/atomgit_atomcode/hydra)
- [MIT License](https://atomgit.com/atomgit_atomcode/hydra/blob/main/LICENSE)
