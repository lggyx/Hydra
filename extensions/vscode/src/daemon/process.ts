...
export class DaemonProcess {
  ...
  private async _ensureRunningImpl(): Promise<boolean> {
    ...
    if (health) {
      ...
      console.log(`[Hydra] Daemon version mismatch: running=${health.version}, expected=${expected}. Restarting...`);

      const shutdownOk = await this.shutdownDaemon();
      if (shutdownOk) {
        console.log('[Hydra] Old daemon stopped successfully');
      } else {
        console.warn(
          `[Hydra] Refusing to start daemon because old daemon ${health.version} is still running; expected ${expected}`
        );
        vscode.window.showWarningMessage(
          `Hydra daemon version mismatch: running ${health.version}, expected ${expected}. Hydra could not stop the old daemon. Please stop the old Hydra daemon or reload VS Code.`
        );
        return false;
      }
      ...
      if (!started) {
        ...
        if (postHealth && postHealth.version === expected) {
          console.log(`[Hydra] Daemon restarted to version ${expected}`);
          return true;
        }
        return false;
      }
      ...
      if (newHealth) {
        console.warn(`[Hydra] New daemon version ${newHealth.version} does not match expected ${expected}`);
      }
      return false;
    }
  ...
  private getExpectedVersion(): string {
    ...
    console.warn('[Hydra] Could not read daemon-version.txt, skipping version check');
    return '';
  }
  ...
  private async shutdownDaemon(): Promise<boolean> {
    ...
    if (this.process && !this.process.killed) {
      console.warn('[Hydra] Graceful shutdown timed out, sending SIGTERM to owned daemon process');
      try {
      ...
      if (!this.process.killed) {
        console.warn('[Hydra] SIGTERM failed, sending SIGKILL to owned daemon process');
        try {
    ...
    console.warn('[Hydra] Daemon did not exit. It may have been started by another process.');
    return false;
  }
  ...
  private async start(): Promise<boolean> {
    ...
    if (!binary) {
      vscode.window.showErrorMessage(
        'Hydra daemon not found for this platform. Reinstall the Hydra extension, install hydra, or set hydra.daemon.binaryPath in settings.'
      );
      return false;
    }
    ...
    vscode.window.showWarningMessage(
      `Hydra daemon started but not responding. Check if port ${port} is available.`
    );
    return false;
  }

  /**
   * Find the hydra binary. Returns the path and args to start the daemon.
   *
   * Search order:
   * 1. User-configured binaryPath
   * 2. Bundled standalone hydra-daemon in the extension package
   * 3. `hydra` in PATH (uses `hydra daemon` subcommand)
  ...
  private findBinary(port: number): DaemonBinary | undefined {
    ...
    // 1. User-configured path (could be hydra or hydra-daemon)
    ...
    // 2. Bundled standalone hydra-daemon binary
    ...
    try {
      const command = process.platform === 'win32' ? 'where hydra' : 'which hydra 2>/dev/null';
    ...
    // 4. Common install locations (hydra main binary with daemon subcommand)
    const hydraPaths = [
      path.join(home, '.hydra', 'bin', 'hydra'),
      path.join(home, '.cargo', 'bin', 'hydra'),
      '/usr/local/bin/hydra',
    ];
    for (const p of hydraPaths) {
      if (fs.existsSync(p)) {
        return { path: p, args: ['daemon', ...portArgs] };
      }
    }

    // 5. Standalone hydra-daemon binary (fallback)
    const daemonPaths = [
      path.join(home, '.hydra', 'bin', 'hydra-daemon'),
      path.join(home, '.cargo', 'bin', 'hydra-daemon'),
      '/usr/local/bin/hydra-daemon',
    ];
    ...
    const devPaths = [
      path.join(workspaceRoot, 'target', 'release', 'hydra-daemon'),
      path.join(workspaceRoot, 'target', 'debug', 'hydra-daemon'),
    ];
    for (const p of devPaths) {
      if (fs.existsSync(p)) {
        console.warn(`[Hydra] Using dev build daemon: ${p}. The bundled daemon was not found — the extension package may be missing resources/bin/<platform>/hydra-daemon.`);
        vscode.window.showWarningMessage(
          `Hydra is using a development build of the daemon (${p}). The bundled daemon was not found. Reinstall the extension or set hydra.daemon.binaryPath in settings.`
        );
        return { path: p, args: portArgs };
      }
    }
  ...
  private findBundledDaemon(): string | undefined {
    ...
    const executable = process.platform === 'win32' ? 'hydra-daemon.exe' : 'hydra-daemon';
