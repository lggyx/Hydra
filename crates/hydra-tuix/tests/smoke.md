<!-- crates/hydra-tuix/tests/smoke.md -->
...
- [ ] `cargo test -p hydra-tuix 2>&1 | tail -5` → all 79 pass
- [ ] `cargo clippy -p hydra-tuix --all-targets 2>&1 | tail -10` → no warnings

## TTY happy path

- [ ] `./target/release/hydra --tuix` shows welcome (logo + model + dir + tips)
...
- [ ] `~/.hydra/history` file exists and is readable
- [ ] Consecutive duplicate messages are collapsed

## Pipe / plain mode

- [ ] `echo "say hi" | ./target/release/hydra --tuix` → no ANSI bytes in output
- [ ] `./target/release/hydra --tuix > out.txt` → `out.txt` is pure text
- [ ] `NO_COLOR=1 ./target/release/hydra --tuix` → no colour; spinner retained on TTY
- [ ] `TERM=dumb ./target/release/hydra --tuix` → no colour, no spinner
