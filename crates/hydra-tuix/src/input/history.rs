// crates/hydra-tuix/src/input/history.rs
...
/// `~/.hydra/image-cache/<hash>.<ext>`. Recorded on submit; consumed
...
impl History {
    ...
    /// Default history path: `~/.hydra/history` on Unix,
    /// `%USERPROFILE%\.hydra\history` on Windows (or a tempdir
