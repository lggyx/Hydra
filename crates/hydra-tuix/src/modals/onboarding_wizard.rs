// crates/hydra-tuix/src/modals/onboarding_wizard.rs
//
...
/// already been issued by the caller, this pushes the Hydra
...
pub struct OnboardingWizard {
    ...
    pub(super) pending_session:
        Option<hydra_core::auth::oauth::LoginSession>,
}
...
impl OnboardingWizard {
    ...
    /// `/language`.
    ///
    /// Synchronously calls [`hydra_core::auth::oauth::start_login`]
    ...
    pub fn new_qr_fast_path() -> Self {
        let (qr_login_url, qr_login_error, pending_session) =
            match hydra_core::auth::oauth::start_login() {
    ...
    pub fn with_initial_language(
        mut self,
        config_lang: Option<hydra_core::locale::Locale>,
    ) -> Self {
        self.language_idx = match config_lang {
            None => 0,
            Some(hydra_core::locale::Locale::En) => 1,
            Some(hydra_core::locale::Locale::ZhCn) => 2,
        };
        self
    }
...
impl OnboardingWizard {
    ...
    pub(super) fn draw_intro_lines(
        ...
        if !compact {
            ...
            content.push(format!("Hydra v{}", env!("CARGO_PKG_VERSION")));
    ...
    pub(super) fn apply_language(
        &self,
        config: &mut hydra_core::config::Config,
    ) -> anyhow::Result<hydra_core::locale::Locale> {
        use hydra_core::locale::Locale;
        ...
        config.save(&hydra_core::config::Config::default_path())?;
        Ok(new_locale)
    }
    ...
    pub(super) fn draw_qr_login_lines(
        ...
        out.push(String::new());
        // Panel title carries the running hydra version so users
        ...
        let panel_title = format!("Hydra · v{}", env!("CARGO_PKG_VERSION"));
        out.extend(draw_panel(
            &panel_title,
            &content,
            "Step 1/1",
        ...
        match outcome {
            ...
            PureOutcome::OpenQrUrlInBrowser => {
                ...
                if let Some(url) = &self.qr_login_url {
                    let _ = hydra_core::auth::oauth::open_browser(url);
                }
                Ok(ModalAction::Continue)
            }
            PureOutcome::RetryQrLogin => {
                ...
                // hydra. Mirrors the constructor — synchronous
            ...
            PureOutcome::ApplySetupThenClose => {
                ...
                // on the regular idle session view (Hydra banner
...
mod tests {
    ...
    fn top_border_has_title() {
        let lines = draw_panel("Hydra", &[], "Step 1/3", 60, true);
        let plain = strip_sgr(&lines[0]);
        assert!(plain.starts_with("┌─ Hydra "));
    ...
    fn narrow_terminal_does_not_panic() {
        // width=10 has inner_width=6 which won't fit "Hydra" title;
        // saturating_sub guards against underflow. We just assert it
        // doesn't panic and produces *some* output.
        let lines = draw_panel("Hydra", &["x".into()], "S", 10, true);
    ...
    fn intro_compact_drops_logo() {
        ...
        // line `Hydra vX.Y.Z` + tagline.
        assert!(joined.contains("Hydra v"));
    ...
    /// the global locale + persists to disk under an HYDRA_HOME
    /// override so tests don't touch real `~/.hydra`.
    #[test]
    fn apply_language_writes_config_and_sets_locale() {
        use hydra_core::locale::Locale;
        let _g = crate::i18n::test_lock();
        let tmp = tempfile::TempDir::new().unwrap();
        // HYDRA_HOME drives Config::config_dir() ahead of $HOME, so
        ...
        let prev_hydra_home = std::env::var("HYDRA_HOME").ok();
        ...
        // Restore env.
        match prev_hydra_home {
            Some(v) => std::env::set_var("HYDRA_HOME", v),
            None => std::env::remove_var("HYDRA_HOME"),
        }
    }
    ...
    fn apply_language_auto_clears_config_field() {
        ...
        let _g = crate::i18n::test_lock();
        let tmp = tempfile::TempDir::new().unwrap();
        let prev = std::env::var("HYDRA_HOME").ok();
        std::env::set_var("HYDRA_HOME", tmp.path());
    ...
    fn blank_config_for_test() -> hydra_core::config::Config {
        hydra_core::config::Config {
    ...
    fn vterm_step1_compact_below_22_rows_drops_logo() {
        ...
        // Compact substitutes `Hydra vX.Y.Z`.
        assert!(screen.contains("Hydra v"));
        assert!(screen.contains("Press Enter to continue"));
    }
    ...
    fn draw_setup_lines_ascii_fallback_produces_pure_ascii_box() {
        let _g = hydra_core::i18n::test_lock();
        hydra_core::i18n::set_locale(hydra_core::i18n::Locale::En);
