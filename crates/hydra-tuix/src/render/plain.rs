// crates/hydra-tuix/src/render/plain.rs
...
/// `HYDRA_PLAIN=1` user opt-in. No raw-mode dependencies, no
...
pub struct PlainRenderer<W: Write + Send> {
    ...
    /// `HYDRA_PLAIN=1`) so downstream branches consistently take
    ...
    fn render(&mut self, line: UiLine) {
        match line {
            ...
            UiLine::User(text) => {
                ...
                // HYDRA_PLAIN=1 on a real TTY): cooked-mode kernel
...
mod tests {
    ...
    /// Real-TTY force_plain (JediTerm / conhost / HYDRA_PLAIN=1):
