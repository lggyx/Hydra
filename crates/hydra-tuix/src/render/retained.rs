// crates/hydra-tuix/src/render/retained.rs
//
// Retained-mode `Renderer` implementation — the alternative to
// `AnsiRenderer`. Enabled by `HYDRA_TUIX_RETAINED=1` (dual-track
// until Phase 6).
//
...
/// `HYDRA_RENDER_DUMP=/path` is set at startup, every byte the
...
impl RetainedRenderer<StdoutTap> {
    pub fn new(caps: TerminalCaps) -> Self {
        ...
        let mirror = std::env::var("HYDRA_RENDER_DUMP")
            .ok()
            .filter(|s| !s.is_empty())
...
impl<W: Write + Send> RetainedRenderer<W> {
    pub fn with_writer(mut out: W, caps: TerminalCaps, w: u16, h: u16) -> Self {
        ...
        // doesn't remain visible above the hydra viewport and mix with
        // the hydra session transcript. `\x1b[3J` only affects scrollback;
        ...
        // handling — matches Claude Code's UX model. Trade-off: hydra's
        ...
        // process that exited weirdly, a prior hydra run that
    ...
    fn emit_body_line_inner(&mut self, row: &[Cell], _bottom: u16) {
        ...
        // the session show nothing above the hydra frame.
        //
    ...
    fn push_body_row(&mut self, row: Vec<Cell>) {
        ...
        // HYDRA_TUIX_LOG=/path. Snippet is the first ~40 chars of
...
impl<W: Write + Send> Renderer for RetainedRenderer<W> {
    ...
    fn shutdown(&mut self) {
        ...
        // terminal returns to default mouse behavior when hydra exits.
...
mod tests {
    ...
    fn build_top_rule_with_badge_renders_session_name_in_reverse_cyan() {
        ...
        r.caps.colors = true;
        r.caps.unicode_symbols = true;
        let row = r.build_top_rule_with_badge(60, Some("hydra加解密"));
...
";
    ...
    fn retained_inflight_tool_does_not_grow_terminal_output_across_ticks() {
        ...
        let detail = "cd /Users/theo/Documents/workspace/hydra && cargo build 2>&1 | tail -5";
    ...
    fn retained_inflight_tool_hides_terminal_cursor() {
        let term_w: u16 = 80;
        let (mut r, buf) = new_capturing(term_w, 24);
        let detail = "cd /Users/theo/Documents/workspace/hydra && cargo check 2>&1 | tail -80";
    ...
    fn retained_welcome_lines_render_via_vterm() {
        ...
        let found_brand = (0..=6).any(|r| vterm.row_text(r).contains("Hydra"));
    ...
    /// at screen.width), but "Hydra" / cwd / model lines still
    ...
    fn retained_resize_preserves_welcome_via_vterm() {
        ...
        let pre_has = (0..24).any(|r| vterm.row_text(r).contains("Hydra"));
        assert!(
            pre_has,
        ...
        let post_has = (0..16).any(|r| vterm.row_text(r).contains("Hydra"));
        assert!(
            post_has,
    ...
    fn retained_resize_reflows_welcome_brand_row_when_expanding() {
        ...
        let brand_row = (0..18)
            ...
            .find(|row| row.contains("Hydra"))
            .expect("brand row should remain visible after widening");
        let atom_idx = brand_row.find("Hydra").unwrap();
    ...
    fn retained_welcome_reflows_path_model_and_hints_on_narrow_terminal() {
        ...
        r.render(UiLine::Welcome {
            ...
            working_dir: "~/workspace/gitcode_project/hydra_family/hydra".into(),
        });
        r.render(UiLine::InputPrompt {
            buf: String::new(),
            cursor_byte: 0,
        ...
        assert!(
            (0..26).any(|row| vterm.row_text(row).contains("Hydra")),
    ...
    fn retained_tool_result_wrap_continuation_has_no_arrow() {
        ...
        let status = status_basic();
        let long_summary =
            "Created new file /tmp/hydra-smoke-temp-check.txt (15 bytes, 1 line)";
        r.render(UiLine::ToolResult {
            success: true,
    ...
    fn retained_large_frame_splits_into_512b_chunks() {
        ...
        r.render(UiLine::Welcome {
            model: "glm-5".into(),
            working_dir: "~/project/hydra".into(),
        });
    ...
    fn retained_clear_screen_then_welcome_renders_via_vterm() {
        ...
        drain_into_vterm(&buf, &mut vterm);
        assert!(
            (0..24).any(|row| vterm.row_text(row).contains("Hydra")),
        ...
        let still_has = (0..24)
            .filter(|row| vterm.row_text(*row).contains("Hydra"))
            .count();
        assert_eq!(
            still_has,
            1,
    ...
    fn retained_resume_from_external_clears_and_forces_repaint() {
        ...
        // emit that restores Hydra on the grid.
        r.render(UiLine::Welcome {
            model: "glm-5".into(),
            working_dir: "~/project/hydra".into(),
        });
    ...
    fn retained_menu_close_leaves_no_welcome_ghost() {
        ...
        let brand_rows = (0..24)
            .filter(|r| vterm.row_text(*r).contains("Hydra"))
            .count();
        assert_eq!(
            brand_rows, 1,
    ...
    fn retained_footer_growth_does_not_duplicate_scrollback() {
        ...
        r.render(UiLine::Welcome {
            ...
            working_dir: "~/Documents/workspace/hydra".into(),
        });
        for i in 0..20 {
    ...
    fn retained_with_writer_does_not_enable_mouse_capture() {
        ...
        // from a prior process or panicked hydra run).
    ...
    /// hydra session showing history above the live viewport.
    #[test]
    ...
    fn retained_approval_pending_no_duplicate_body_rows() {
        ...
        r.render(UiLine::ToolCallInFlight {
            id: "call-7".into(),
            name: "WriteFile".into(),
            detail: "hydra_smoke_replace.txt".into(),
        });
    ...
    fn retained_footer_grow_then_push_catches_up_no_footer_overlap() {
        ...
        let whoami_text =
            "  TheoCui (saulcy)\n  cuizk@csdn.net\n  auth: /Users/theo/.hydra/auth.toml\n";
