// crates/hydra-tuix/src/event_loop/usage_monitor.rs
//
...
use hydra_core::coding_plan::types::UsageInfo;
use tokio::sync::mpsc;
...
pub fn spawn_check(slot: Arc<Mutex<Option<UsageInfo>>>, wake_tx: mpsc::Sender<()>) {
    tokio::spawn(async move {
        ...
        let fetched: Result<UsageInfo, ()> = tokio::task::spawn_blocking(|| {
            let client = hydra_core::coding_plan::client::Client::from_stored_auth()
                .map_err(|_| ())?;
...
///   - `HYDRA_USAGE_DEBUG_THRESHOLD=N` lowers the 80.0 gate to `N`,
///     so the hint shows at any real percent ≥ N. Used to verify the
///     hint is wired up without actually consuming 80%+ of quota.
///   - `HYDRA_USAGE_DEBUG_PERCENT=N` overrides the live percent with
...
fn build_usage_hint_from_info(info: &UsageInfo) -> Option<(String, HintSeverity)> {
    let raw_percent = std::env::var("HYDRA_USAGE_DEBUG_PERCENT")
        .ok()
        .and_then(|v| v.parse::<f64>().ok())
        .unwrap_or(info.usage_percent);

    let threshold = std::env::var("HYDRA_USAGE_DEBUG_THRESHOLD")
        .ok()
        .and_then(|v| v.parse::<f64>().ok())
        .unwrap_or(80.0);
