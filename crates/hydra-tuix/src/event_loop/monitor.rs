// crates/hydra-tuix/src/event_loop/monitor.rs
//
...
pub fn spawn_check(
    config_snapshot: hydra_core::config::Config,
    ...
    tokio::spawn(async move {
        ...
        let fetch: Result<Vec<String>, ()> = tokio::task::spawn_blocking(move || {
            let client =
                hydra_core::coding_plan::client::Client::from_stored_auth().map_err(|_| ())?;
            ...
            let models = client
                .list_models_v2(hydra_core::coding_plan::PlanType::Max)
                .map_err(|_| ())?;
            Ok(models
                .into_iter()
