// crates/hydra-tuix/src/platform.rs
//
...
fn collapse_home_with(path: &str, home: Option<&std::path::Path>) -> String {
    ...
    if let Some(home) = home {
        ...
        if !home_str.is_empty() {
            ...
            // `~/hydra` instead of the raw `C:\USERS\alice\hydra`.
            ...
            if let Some(rest) = rest {
                ...
                // docs) which all reference `~/.hydra/...`.
...
/// Uses HYDRA_HOME if set, otherwise falls back to home directory.
pub fn history_path() -> PathBuf {
    hydra_core::config::Config::config_dir().join("history")
}
...
pub fn image_cache_dir() -> PathBuf {
    hydra_core::config::Config::config_dir().join("image-cache")
}
...
mod tests {
    ...
    fn collapse_home_emits_forward_slash_on_windows_separators() {
        if let Some(home) = home_dir() {
            ...
            let nested = format!("{home_str}{sep}hydra{sep}src");
            let got = collapse_home(&nested);
            assert_eq!(got, "~/hydra/src");
    ...
    /// `\\?\C:\Users\username\hydra` — note the `\\?\` prefix that
    ...
    /// path is shown instead of `~/hydra`.
    #[test]
    fn collapse_home_windows_case_insensitive_match() {
        ...
        assert_eq!(
            collapse_home_with(r"C:\Users\username\hydra", Some(home)),
            "~/hydra"
        );

        // Home dir uses `C:\Users\username`, canonicalize returns
        // `C:\USERS\username` — must still collapse to `~/hydra`.
    ...
    fn image_cache_dir_lives_under_config_dir() {
        ...
        let cfg = hydra_core::config::Config::config_dir();
