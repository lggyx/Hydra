//! Event and Envelope schema for Hydra telemetry v2.
//!
//! The events are: open_hydra, llm_chat, tool_call, use_command,
...
pub enum Event {
    /// Fired when Hydra is launched (interactive CLI, oneshot, or TUI entry).
    ...
    /// Final event before user opts out via `hydra telemetry disable`.
    ...
    /// open-source build of Hydra attempts to send a request to the
...
mod tests {
    ...
    fn record_flattens_envelope_and_event() {
        ...
        assert_eq!(v["event_id"], "open_hydra");
