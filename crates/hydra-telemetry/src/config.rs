...
pub fn resolve(
    ...
    let state = if env.var("HYDRA_TELEMETRY").as_deref() == Some("0") {
        TelemetryState::Disabled("env:HYDRA_TELEMETRY=0")
...
mod tests {
    ...
    fn env_wins_over_config() {
        ...
        let r = resolve(
            ...
            &CliOverride::default(),
            dir(),
            &env(&[("HYDRA_TELEMETRY", "0")]),
        );
        assert_eq!(r.state.reason(), Some("env:HYDRA_TELEMETRY=0"));
    }
    ...
    fn endpoint_env_override() {
        let r = resolve(
            ...
            &CliOverride::default(),
            dir(),
            &env(&[("HYDRA_TELEMETRY_ENDPOINT", "https://test.example/v1")]),
        );
