...
/// Build a fake hydra data dir so the CLI sees something to scan.
///
/// Under unified semantics, `HYDRA_HOME` IS the data root (equivalent to
/// `~/.hydra/`), so we point the env var directly at this dir — no extra
/// `.hydra` subdir.
fn make_fake_data(tmp: &TempDir) -> std::path::PathBuf {
    let data = tmp.path().join("hydra-data");
...
// target/debug/hydra), and deleting it would break subsequent tests in the same
// run. Coverage for end-to-end deletion is in hydra-core's execute_tests via
// NoopSelfDelete (see crates/hydra-core/src/uninstall/actions.rs).
