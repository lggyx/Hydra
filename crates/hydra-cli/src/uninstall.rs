...
pub fn run(args: Args) -> anyhow::Result<()> {
    ...
    if args.purge && args.keep_data {
        eprintln!("hydra uninstall: --purge conflicts with --keep-data");
    ...
    let decisions = match mode {
        ...
        DecisionMode::AbortNoTty => {
            eprintln!(
                "hydra uninstall: refusing to run interactively without a TTY.\n\
    ...
    if !final_decisions.binary {
        eprintln!("hydra uninstall: cannot uninstall without removing binary; aborted.");
    ...
    let outcome =
        hydra_core::uninstall::execute(&plan, final_decisions, strategy.as_ref(), Some(ctx))?;
...
fn confirm_and_kill_running_processes() -> anyhow::Result<bool> {
    use hydra_core::uninstall::actions::{kill_process, list_hydra_processes};
    ...
    println!("\nFound {} running hydra process(es):", procs.len());
    for p in &procs {
...
fn print_plan(plan: &hydra_core::uninstall::scan::Plan, decisions: Decisions) {
...
fn print_group(
    plan: &hydra_core::uninstall::scan::Plan,
    g: Group,
    label: &str,
    will_remove: bool,
) {
...
fn prompt_user(plan: &hydra_core::uninstall::scan::Plan) -> anyhow::Result<Option<Decisions>> {
    use std::io::{BufRead, Write};

    println!("This will uninstall Hydra from your system.\n");
...
fn summarize_decision(plan: &hydra_core::uninstall::scan::Plan, g: Group, will_remove: bool) {
...
fn build_context(plan: &hydra_core::uninstall::scan::Plan) -> anyhow::Result<ExecuteContext> {
    ...
    #[cfg(unix)]
    {
        let prefix = plan
            ...
            .unwrap_or_default();
        let rc = hydra_core::uninstall::paths::unix_rc_paths();
