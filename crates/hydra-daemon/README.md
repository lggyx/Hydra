...
# 指定端口
cargo run -p hydra-daemon -- --port 8080

# 指定绑定 IP（允许外部访问）
cargo run -p hydra-daemon -- --host 0.0.0.0

# 同时指定 host 和 port
cargo run -p hydra-daemon -- --host 0.0.0.0 --port 8080
```
...
| 环境变量 | 说明 |
|----------|------|
| `HYDRA_DAEMON_ENABLE_DANGEROUS_TOOLS` | 设为 `1` 启用 bash 和写文件的 daemon 工具 |
| `HYDRA_DISABLE_TOOLS` | 逗号分隔的工具名列表，禁用指定工具（如 `bash,write_file`） |
...
```json
{
  "path": "/home/user/.hydra/config.toml",
...
{
  "logged_in": true,
  "auth_path": "/home/user/.hydra/auth.json",
  "user": {
...
daemon 聊天模式下注册的工具（可通过 `HYDRA_DISABLE_TOOLS` 环境变量禁用）：
