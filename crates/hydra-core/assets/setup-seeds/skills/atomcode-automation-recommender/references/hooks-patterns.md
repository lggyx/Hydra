...
Hooks automatically run commands in response to Hydra events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when Hydra sends notifications. Use matchers to filter by notification type.

### Permission Alerts
| Matcher | Use Case |
|---------|----------|
| `permission_prompt` | Alert when Hydra requests permissions |
...
| Matcher | Use Case |
|---------|----------|
| `idle_prompt` | Alert when Hydra is waiting for input (60+ seconds idle) |

**Recommend**: Play sound or send notification when Hydra needs attention
**Value**: Know when Hydra is ready for your input
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"Hydra is waiting\" with title \"Hydra\"'"
          }
        ]
      }
    ]
...
|---------|---------------|
| `permission_prompt` | Hydra needs permission for a tool |
| `idle_prompt` | Hydra waiting for input (60+ seconds) |
...
Hooks go in `.hydra/settings.json`:
...
Recommend creating the `.hydra/` directory if it doesn't exist.
