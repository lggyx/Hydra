...
Skills are packaged expertise with workflows, reference materials, and best practices. Create them in `.hydra/skills/<name>/SKILL.md`. Skills can be invoked by Hydra automatically when relevant, or by users directly with `/skill-name`.

Some pre-built skills are available through the setup installer (`hydra setup`).
...
### Built-in Skills (installed via `hydra setup`)
...
### Built-in Commands (installed via `hydra setup`)
...
|-------|--------|---------|----------|
| **ask** | hydra | `/plugin marketplace add https://atomgit.com/atomgit_atomcode/hydra-skills` then `/plugin install hydra@hydra-skills` | Hydra usage & docs Q&A — installation, config, slash commands, troubleshooting |

---

## Custom Project Skills

Create project-specific skills in `.hydra/skills/<name>/SKILL.md`.
...
user-invocable: false           # Only Hydra can invoke (for background knowledge)
...
3. Run validation: `bash ~/.hydra/skills/create-migration/scripts/validate-migration.sh`
...
### Project Conventions (Hydra-only)

Background knowledge Hydra applies automatically:
...
The command output replaces the placeholder before Hydra sees the skill content.
