...
MCP (Model Context Protocol) servers extend Hydra's capabilities by connecting to external tools and services.
...
2. **Global config** (`~/.hydra/mcp.json`) - Available across all projects
...
**Debugging**: Use `hydra --mcp-debug` to identify configuration issues.

## Documentation & Knowledge

### context7
**Best for**: Projects using popular libraries/SDKs where you want Hydra to code with up-to-date documentation
...
**Value**: Hydra fetches live documentation instead of relying on training data, reducing hallucinated APIs and outdated patterns.
...
**Value**: Hydra can interact with your running app, take screenshots, fill forms, and verify UI behavior.
...
**Value**: Hydra can query tables, manage auth, and interact with Supabase storage directly.
...
**Value**: Hydra can create issues, review PRs, check workflow runs, and manage releases.
...
**Value**: Hydra can investigate Sentry issues, find root causes, and suggest fixes.
...
**Value**: Hydra remembers project context, decisions, and patterns across conversations.
