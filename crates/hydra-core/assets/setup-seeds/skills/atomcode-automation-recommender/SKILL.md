---
name: setup
description: Analyze a codebase and recommend Hydra automations (hooks, subagents, skills, plugins, MCP servers). Use when user asks for automation recommendations, wants to optimize their Hydra setup, mentions improving Hydra workflows, asks how to first set up Hydra for a project, or wants to know what Hydra features they should use.
user_invocable: true
argument_hint: "[focus area, e.g. hooks, mcp, skills, all]"
allowed_tools: Read, Glob, Grep, Bash, Write, Edit
---

# Hydra Automation Recommender

Analyze codebase patterns to recommend tailored Hydra automations, then **offer to install them**.
...
|------|----------|----------------|
| **Skills** | Packaged expertise, workflows, repeatable tasks | `npx skills add <pkg>` or create `.hydra/skills/<name>/SKILL.md` |
| **Plugins** | Collections of skills, commands, agents, hooks | `/plugin marketplace add <url>` then `/plugin install <name>` |
| **MCP Servers** | External tool integrations (databases, APIs, docs) | Write to `.hydra/mcp.json` or `~/.hydra/mcp.json` |
| **Hooks** | Automatic actions on tool events (format, lint, block) | Write to `.hydra/settings.json` |
| **Subagents** | Specialized reviewers (security, performance, a11y) | Create `.hydra/skills/<name>/SKILL.md` with reviewer prompt |
| **Commands** | Quick slash commands (/test, /review, /deploy) | Create `.hydra/commands/<name>.md` |
...
# Check existing Hydra config
ls -la .hydra/ CLAUDE.md .mcp.json 2>/dev/null
...
|-----------------|-------|-----------------|------------|
| Any project (Hydra Q&A) | **ask** | `/plugin marketplace add https://atomgit.com/atomgit_atomcode/hydra-skills` then `/plugin install hydra@hydra-skills` | Both |
...
| Code style | **project-conventions** | Hydra-only |
...
In Hydra, subagents are implemented as skills with specialized reviewer prompts:
...
```markdown
## Hydra Automation Recommendations
...
#### [hook name]
**Why**: [specific reason]
**Config**: Add to `.hydra/settings.json`
...
#### [agent name]
**Why**: [specific reason]
**Create**: `.hydra/skills/[name]/SKILL.md`
...
2. 复制 SKILL.md 内容
3. 创建 ~/.hydra/skills/[name]/SKILL.md
```

**Skills (custom creation):**
Use the Write tool to create `.hydra/skills/<name>/SKILL.md` with the recommended content.

**MCP Servers:**
Use the Write/Edit tool to add the server config to `.hydra/mcp.json` or `~/.hydra/mcp.json`:
```bash
# Read existing config
cat .hydra/mcp.json 2>/dev/null || echo '{}'
# Merge new server config
```

**Hooks:**
Use the Write/Edit tool to add hook config to `.hydra/settings.json`:
```bash
cat .hydra/settings.json 2>/dev/null || echo '{}'
```

**Subagents (as skills):**
Use the Write tool to create `.hydra/skills/<name>/SKILL.md` with the reviewer prompt from the subagent template.

**Commands:**
Use the Write tool to create `.hydra/commands/<name>.md`.
...
已安装：
  ✓ skill: [name] — [source]
  ✓ mcp: [name] — 写入 .hydra/mcp.json（重启 Hydra 后生效）
  ✓ hook: [name] — 写入 .hydra/settings.json
  ✓ subagent: [name] — 创建 .hydra/skills/[name]/SKILL.md

💡 MCP 服务需要重启 Hydra 后生效。
...
- `user_invocable: false` — Hydra-only (for background knowledge)
...
替代方案：手动创建 skill 文件
  mkdir -p ~/.hydra/skills/[name]
  # 然后将 SKILL.md 内容粘贴进去
```

### .hydra directory not writable
```
⚠️ 无法写入 .hydra/ 目录。
...
或手动创建：
  mkdir -p .hydra/skills .hydra/commands
```
...
Configure allowed tools in `.hydra/settings.json`:
```json
{
  "permissions": {
