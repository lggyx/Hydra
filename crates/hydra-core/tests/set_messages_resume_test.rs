...
//! mechanism that powers both `/resume` and `hydra -c` session restoration.
//!
//! Issue #293: `hydra -c` could not fully restore model context because
...
use hydra_core::conversation::turn::{TurnStatus, TurnTracker};
use hydra_core::conversation::Conversation;
use hydra_core::tool::{ToolCall, ToolResult};
...
fn context_builds_with_turn_tracking_after_set_messages() {
    ...
    use hydra_core::ctx::CtxBuilder;
    use hydra_core::ctx::DefaultCtx;
