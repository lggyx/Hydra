...
//!
//! 这些测试通过 `HYDRA_HOME` 环境变量把 `Config::config_dir()` 重定向到
//! tempdir,确保不会污染真实的 `~/.hydra/`。Env var 是进程全局的,所以全部
//! 测试都用 `#[serial]` 序列化。

use hydra_core::setup::{self, RunOptions};
use serial_test::serial;
use std::path::Path;

/// 在 tempdir 项目中跑 setup,同时把 `HYDRA_HOME` 指向另一个 tempdir。
    ...
    G: FnOnce(&mut RunOptions),
{
    ...
    let old = std::env::var_os("HYDRA_HOME");
    std::env::set_var("HYDRA_HOME", user.path());
...
fn second_run_skips_already_installed() {
    ...
    if !report1.summary.installed.is_empty() {
        let has_already = report2.summary.skipped.iter().any(|(_, reason)| {
            matches!(
                reason,
                hydra_core::setup::install::SkipReason::AlreadyInstalled
            )
        });
        assert!(
            has_already,
...
fn concurrent_runs_second_fails_lock() {
    ...
    let (r1, r2) = std::thread::scope(|s| {
        let t1 = s.spawn(|| {
            let o = hydra_core::setup::RunOptions::new(proj_a);
            hydra_core::setup::run(o)
        });
        ...
        let t2 = s.spawn(|| {
            let o = hydra_core::setup::RunOptions::new(proj_b);
            hydra_core::setup::run(o)
        });
    ...
    for r in [&r1, &r2] {
        if let Err(e) = r {
            assert!(
                matches!(
                    e,
                    hydra_core::setup::SetupError::LockHeld { .. }
                        | hydra_core::setup::SetupError::LockIo(_)
                ),
