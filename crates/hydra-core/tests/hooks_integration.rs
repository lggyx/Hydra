...
use hydra_core::hook::executor::HookExecutor;
use hydra_core::hook::{HookConfig, HookContext, HookEvent, PreHookResult};
...
async fn pre_hook_blocks_dangerous_command() {
    // Shell script that inspects HYDRA_HOOK_CONTEXT for "rm -rf".
    // If the context contains it, the hook blocks; otherwise it allows.
    let script = r#"
        if echo "$HYDRA_HOOK_CONTEXT" | grep -q 'rm -rf'; then
...
async fn post_hook_receives_result() {
    ...
    let marker = format!("/tmp/hydra_hook_test_post_{pid}");

    // The hook writes HYDRA_TOOL_NAME into a temp file.
    let script = format!(r#"echo "$HYDRA_TOOL_NAME" > {marker}"#);
    ...
    assert_eq!(
        content.trim(),
        "write_file",
        "post-hook should receive the tool name via HYDRA_TOOL_NAME"
    );
...
async fn hook_context_json_contains_expected_fields() {
    let pid = std::process::id();
    let marker = format!("/tmp/hydra_hook_test_ctx_{pid}");

    // Write the full HYDRA_HOOK_CONTEXT to a file so we can inspect it.
    let script = format!(r#"echo "$HYDRA_HOOK_CONTEXT" > {marker}"#);
    ...
    let parsed: serde_json::Value = serde_json::from_str(content.trim())
        .unwrap_or_else(|e| panic!("HYDRA_HOOK_CONTEXT is not valid JSON: {e}"));
