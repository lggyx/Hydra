...
use hydra_core::tool::{Tool, ToolContext};
use std::path::PathBuf;

async fn run_grep(pattern: &str, path: &str, wd: &str) -> hydra_core::tool::ToolResult {
    let tool = GrepTool;
...
async fn run_grep_no_path(pattern: &str, wd: &str) -> hydra_core::tool::ToolResult {
    let tool = GrepTool;
    let ctx = ToolContext::new(PathBuf::from(wd));
    let args = serde_json::json!({
        "pattern": pattern,
