// crates/hydra-core/src/config/instructions.rs
//
// Three-tier layered instruction system: global → project → user.
//
// 1. Global:  ~/.hydra/ATOMCODE.md — personal preferences across all projects
// 2. Project: <project>/.hydra.md, ATOMCODE.md, CLAUDE.md, or claude.md
//             (first match wins; CLAUDE.md/claude.md are accepted for
//             compatibility with projects migrating from Claude Code)
// 3. User:    <project>/.hydra.user.md — personal per-project, in .gitignore
...
impl LayeredInstructions {
    ...
    ///
    /// - Global:  `~/.hydra/ATOMCODE.md`
    /// - Project: `<project_root>/.hydra.md`, `ATOMCODE.md`, `CLAUDE.md`,
    ///   or `claude.md` (first match wins, in that order)
    /// - User:    `<project_root>/.hydra.user.md`
    pub fn load(project_root: &Path) -> Self {
        ...
        let project = [".hydra.md", "ATOMCODE.md", "CLAUDE.md", "claude.md"]
            .iter()
            .find_map(|name| Self::try_load(&project_root.join(name), InstructionLevel::Project));

        let user =
            Self::try_load(&project_root.join(".hydra.user.md"), InstructionLevel::User);
...
mod tests {
    ...
    fn empty_dir_produces_no_instructions() {
        ...
        let instructions = LayeredInstructions {
            ...
            project: LayeredInstructions::try_load(
                &tmp.path().join(".hydra.md"),
                InstructionLevel::Project,
            ),
            user: LayeredInstructions::try_load(
                &tmp.path().join(".hydra.user.md"),
                InstructionLevel::User,
            ),
        };
    ...
    fn project_atomcode_md_is_found() {
        let tmp = tempfile::tempdir().unwrap();
        fs::write(tmp.path().join(".hydra.md"), "Use tabs.").unwrap();
    ...
    #[test]
    fn hydra_md_preferred_over_claude_md() {
        let tmp = tempfile::tempdir().unwrap();
        fs::write(tmp.path().join(".hydra.md"), "hydra wins").unwrap();
        fs::write(tmp.path().join("CLAUDE.md"), "claude loses").unwrap();
        let instructions = LayeredInstructions::load(tmp.path());
        let project = instructions.project.expect("project tier should load");
        assert!(project.content.contains("hydra wins"));
    }

    #[test]
    fn hydra_uppercase_preferred_over_claude_md() {
        let tmp = tempfile::tempdir().unwrap();
    ...
    fn lowercase_preferred_over_uppercase() {
        let tmp = tempfile::tempdir().unwrap();
        fs::write(tmp.path().join(".hydra.md"), "lowercase wins").unwrap();
        fs::write(tmp.path().join("ATOMCODE.md"), "uppercase loses").unwrap();

        // Simulate the load logic: .hydra.md is checked first.
        let project = [".hydra.md", "ATOMCODE.md"]
            .iter()
            .find_map(|name| {
    ...
    fn user_instructions_loaded() {
        let tmp = tempfile::tempdir().unwrap();
        fs::write(tmp.path().join(".hydra.user.md"), "my prefs").unwrap();
    ...
    fn empty_file_is_skipped() {
        let tmp = tempfile::tempdir().unwrap();
        fs::write(tmp.path().join(".hydra.md"), "   \n  \n").unwrap();
    ...
    fn merged_with_only_project_produces_single_section() {
        let instructions = LayeredInstructions {
            global: None,
            project: Some(InstructionFile {
                path: PathBuf::from("/project/.hydra.md"),
