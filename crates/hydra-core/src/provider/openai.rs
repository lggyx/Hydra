...
fn build_codingplan_headers(
    ...
    // the auth check, so an official user with no `~/.hydra/auth.toml`
...
impl LlmProvider for OpenAiProvider {
    fn chat_stream(
        ...
        // Set HYDRA_WIRE_DUMP=1 to dump every outbound LLM request body
        // to ~/.hydra/wire-dump/<timestamp>.json so we can verify what
        ...
        // snapshot proves whether hydra or the proxy is the source of
        // truncation. Remove once root-caused.
        if std::env::var("HYDRA_WIRE_DUMP").ok().as_deref() == Some("1") {
            if let Ok(home) = std::env::var("HOME") {
                let dir = std::path::PathBuf::from(home).join(".hydra/wire-dump");
        ...
        tokio::spawn(async move {
            ...
            if !tail.is_empty() {
                let _ = tx.send(Ok(StreamEvent::Error(format!(
                    "Provider returned a non-SSE response Hydra could not parse: {}",
                ...
                //
                // Observed three times across May 2026 atomgr/hydra
...
mod tests {
    ...
    fn truncation_detector_flags_gitcode_real_world_ratio() {
        ...
        // body hydra counted at ~78K content chars. Ratio 12.58.
