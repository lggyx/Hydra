...
fn generate_commit_message(files: &[String]) -> String {
    ...
    if file_count == 1 {
        format!("hydra: edit {}", short_names[0])
    } else if file_count <= 3 {
        format!("hydra: edit {}", short_names.join(", "))
    } else {
        format!(
            "hydra: edit {} and {} more",
            short_names[..2].join(", "),
...
mod tests {
    ...
    fn init_repo() -> tempfile::TempDir {
        ...
        run_git(
            dir.path(),
            &["config", "user.email", "hydra@example.com"],
        );
        run_git(dir.path(), &["config", "user.name", "Hydra"]);
        dir
    }
    ...
    fn auto_commit_commits_only_when_index_is_clean() {
        ...
        let log = Command::new("git")
            ...
            .output()
            .unwrap();
        assert!(String::from_utf8_lossy(&log.stdout).contains("hydra: edit edited.txt"));
    }
