...
pub fn scan(binary_path: &Path, hydra_dir: &Path) -> Result<Plan> {
    ...
    if let Some(dir) = binary_path.parent() {
        // Self-update backup uses extension `.bak` appended (hydra.bak / hydra.exe.bak).
        let bak_name = {
        ...
        for (name, note) in [
            (".hydra.rolling", "self-update rename slot"),
            (".hydra.download", "self-update partial download"),
            (".hydra.writable-probe", "self-update probe leftover"),
        ] {
            let p = dir.join(name);
            if p.exists() {
    ...
    for dname in m.state_dirs {
        let p = hydra_dir.join(dname);
        if p.exists() {
            items.push(item(Group::State, p, dname)?);
        }
    }
    if hydra_dir.exists() {
        for entry in fs::read_dir(hydra_dir)? {
            let entry = entry?;
    ...
    Ok(Plan {
        ...
        hydra_dir: hydra_dir.to_path_buf(),
    })
}
...
fn needs_privilege_to_remove(p: &Path) -> bool {
    ...
    let c_path = match std::ffi::CString::new(parent.as_os_str().as_bytes()) {
        Ok(s) => s,
        Err(_) => return true, // path contains an interior NUL 鈥?treat conservatively
    };
...
mod tests {
    ...
    fn make_fake_install(tmp: &TempDir) -> (std::path::PathBuf, std::path::PathBuf) {
        ...
        let exe = bin_dir.join("hydra");
        fs::write(&exe, b"\x7fELF......").unwrap();
        // self-update artifacts
        fs::write(bin_dir.join("hydra.bak"), b"old").unwrap();
        fs::write(bin_dir.join(".hydra.rolling"), b"r").unwrap();
    ...
    fn scan_finds_binary_and_artifacts() {
        ...
        assert!(bin_paths.contains(&exe.with_file_name("hydra.bak")));
        assert!(bin_paths.contains(&exe.with_file_name(".hydra.rolling")));
    }
