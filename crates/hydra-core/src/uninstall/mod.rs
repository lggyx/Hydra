//! Uninstall flow shared between the `hydra uninstall` subcommand
...
pub struct ExecuteContext {
    ...
    /// Literal install dir string for Windows registry filter (e.g., "%LOCALAPPDATA%\\Hydra").
    #[cfg(windows)]
    pub windows_install_dir_literal: Option<String>,
    /// Expanded install dir string for Windows registry filter (e.g., "C:\\Users\\theo\\AppData\\Local\\Hydra").
    #[cfg(windows)]
...
pub fn execute(
    ...
    // --- 3. rmdir $HYDRA_HOME/ if empty ---
    if plan.hydra_dir.exists() {
        let _ = std::fs::remove_dir(&plan.hydra_dir); // ignore non-empty error
    }
