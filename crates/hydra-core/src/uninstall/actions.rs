...
/// Remove the canonical `# Added by Hydra installer\nexport PATH="<prefix>:$PATH"`
...
/// `None` otherwise.
pub fn strip_hydra_path_block(content: &str, prefix: &str) -> Option<String> {
    let comment = "# Added by Hydra installer";
...
/// Remove an entry equal to `target_literal` (e.g. `%LOCALAPPDATA%\Hydra`)
/// or `target_expanded` (e.g. `C:\Users\theo\AppData\Local\Hydra`) from a
...
pub fn matches_hydra_name(name: &str) -> bool {
    let stripped = name.strip_suffix(".exe").unwrap_or(name);
    matches!(stripped, "hydra" | "hydra-daemon")
}

/// List all hydra-family processes excluding the calling process.
pub fn list_hydra_processes() -> Vec<ProcessInfo> {
...
/// Read an rc file, strip the Hydra installer block targeting `prefix`,
/// write a `.hydra-uninstall.bak` next to it, then write the cleaned file.
...
pub fn apply_unix_path_cleanup(rc_path: &Path, prefix: &str) -> io::Result<PathCleanupResult> {
    ...
    let new_content = match strip_hydra_path_block(&content, prefix) {
        Some(c) => c,
        None => {
            return Ok(PathCleanupResult {
                modified: false,
...
impl SelfDeleteStrategy for PlatformSelfDelete {
    ...
    fn run(&self, exe: &Path) -> io::Result<()> {
        ...
        // Rename live exe to .hydra.rolling so the install dir can be deleted.
        ...
        // reported in gitcode.com/atomgit_atomcode/hydra/issues/352.
...
#[cfg(test)]
mod path_line_tests {
    use super::strip_hydra_path_block;
...
";
        assert_eq!(
            strip_hydra_path_block(input, PREFIX).as_deref(),
            Some(expect)
        );
    }
    ...
    fn returns_none_when_no_block() {
        ...
        assert_eq!(strip_hydra_path_block(input, PREFIX), None);
    }
...
export PATH=\"/Users/test/.local/bin:$PATH\"
";
        let out = strip_hydra_path_block(input, PREFIX).unwrap();
        assert!(!out.contains("Hydra installer"));
...
mod windows_path_tests {
    ...
    fn strips_exact_match() {
        let path = r"C:\Program Files\Git\cmd;C:\Users\theo\AppData\Local\Hydra;C:\Windows";
        let target = r"C:\Users\theo\AppData\Local\Hydra";
        let expanded = r"C:\Users\theo\AppData\Local\Hydra";
    ...
    #[test]
    fn case_insensitive() {
        let path = r"c:\users\Theo\appdata\local\hydra;C:\Windows";
        let out = strip_path_entry(
            path,
            r"C:\Users\theo\AppData\Local\Hydra",
            r"C:\Users\theo\AppData\Local\Hydra",
        );
    ...
    fn ignores_trailing_backslash() {
        let path = r"C:\Users\theo\AppData\Local\Hydra\;C:\Windows";
        let out = strip_path_entry(
            path,
            r"C:\Users\theo\AppData\Local\Hydra",
            r"C:\Users\theo\AppData\Local\Hydra",
    ...
    fn matches_unexpanded_localappdata() {
        let path = r"%LOCALAPPDATA%\Hydra;C:\Windows";
        let out = strip_path_entry(
            path,
            r"%LOCALAPPDATA%\Hydra",
            r"C:\Users\theo\AppData\Local\Hydra",
        );
    ...
    fn preserves_other_atomcode_substring_entries() {
        // A directory that *contains* Hydra in its name but isn't the install dir.
        let path = r"C:\AtomCodeStuff\bin;C:\Users\theo\AppData\Local\Hydra;C:\Windows";
        let out = strip_path_entry(
            path,
            r"C:\Users\theo\AppData\Local\Hydra",
            r"C:\Users\theo\AppData\Local\Hydra",
...
mod process_tests {
    ...
    fn excludes_self() {
        ...
        let procs = list_hydra_processes();
        for p in procs {
            assert_ne!(p.pid, me);
        }
    }
    ...
    fn name_matcher_recognizes_atomcode_variants() {
        assert!(matches_hydra_name("hydra"));
        assert!(matches_hydra_name("hydra.exe"));
        assert!(matches_hydra_name("hydra-daemon"));
        assert!(matches_hydra_name("hydra-daemon.exe"));
        assert!(!matches_hydra_name("vscode"));
        assert!(!matches_hydra_name("hydra-stuff"));
    }
}
...
mod rc_apply_tests {
    ...
    fn backup_created_and_block_removed() {
        ...
        assert!(res.modified);
        assert!(rc.with_file_name(".zshrc.hydra-uninstall.bak").exists());
        let new = std::fs::read_to_string(&rc).unwrap();
        assert!(!new.contains("Hydra"));
    }
