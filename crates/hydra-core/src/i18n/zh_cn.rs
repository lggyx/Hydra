...
pub(super) fn zh_cn(msg: Msg<'_>) -> Cow<'static, str> {
    match msg {
        ...
        Msg::CpSignVersionTooOld => Cow::Borrowed(
            "当前 Hydra 版本过旧，已不兼容 CodingPlan。请升级 Hydra 后继续使用。",
        ),
        Msg::CpUpgradeRequired => Cow::Borrowed(
            "需要升级才能继续使用 CodingPlan，请前往官方发布页安装最新版 Hydra。",
        ),
...
"#.into(),
        ...
        Msg::McpReloadFailed { error } =>
            format!("MCP 重载失败：无法加载 .mcp.json / $HYDRA_HOME/mcp.json：{:#}", error).into(),
        // /mcp login / logout
        ...
        Msg::HelpCustomCreateHint =>
            "  创建方式：~/.hydra/commands/<名称>.md 或 .hydra/commands/<名称>.md\n".into(),
...
"扫描项目、安装种子文件并运行 setup skill [hooks|mcp|skills|all]".into(),
        ...
        Msg::CmdDescReload => "从磁盘重新加载 $HYDRA_HOME/config.toml".into(),
        Msg::CmdDescCd => "切换工作目录".into(),
Msg::CmdDescInit => "从工作目录生成 .hydra.md 项目指令".into(),
...
Msg::CmdDescBackground => "在隔离的后台上下文中运行一次性任务（只读工具子集）".into(),
        ...
        Msg::CmdDescIssue => "为 Hydra 报告 Bug / 提出功能建议（交互式向导）".into(),
