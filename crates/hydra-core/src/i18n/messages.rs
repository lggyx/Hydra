pub enum Msg<'a> {
    ...
    /// Emitted inline by `/login` and `hydra login` when the stored
    ...
    /// Server returned `HYDRA_SIG_INVALID` AND the alg_version is
    ...
    /// reaching hydra, so Windows builds emit
    ...
    SetupFailedLabel,
    /// Per-item installed row: "  ✓ skill:hydra-automation-recommender → /path"
    ...
    CmdSetupTip,
    /// "Running hydra setup..." — shown while setup is in progress.
    CmdSetupRunning,
