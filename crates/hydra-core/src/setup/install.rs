...
const GITIGNORE_MARKER: &str = ".hydra/local/";
const GITIGNORE_BLOCK: &str = "\n# Hydra local-scope configs (machine-specific)\n.hydra/local/\n";
...
impl InstalledTxn {
    pub fn new(project_root: PathBuf) -> std::io::Result<Self> {
        let ts = std::time::SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let backup_dir = project_root.join(".hydra").join(format!(".setup-backup-{ts}"));
    ...
    /// Append the Hydra local-scope marker to `.gitignore`. Idempotent —
...
mod tests {
    ...
    fn new_creates_backup_dir() {
        ...
        assert!(txn.backup_dir.starts_with(dir.path().join(".hydra")));
        std::mem::forget(txn);
    }
    ...
    fn append_gitignore_adds_local_marker() {
        ...
        assert!(content.contains(".hydra/local/"));
        std::mem::forget(txn);
    }
