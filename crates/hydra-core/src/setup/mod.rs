//! Setup wizard — install seed files (skills/commands/hooks/MCP) to `$HYDRA_HOME/`.
//!
...
pub fn run(opts: RunOptions) -> SetupResult<SetupReport> {
    ...
    // Install directory-style skills (e.g., hydra-automation-recommender/).
    install_directory_skills_from_seeds(&cache_dir, &mut summary, opts.force);

    // Append .gitignore marker for .hydra/local/.
    ...
    let state_data = state::SetupState {
        ...
        hydra_version: env!("CARGO_PKG_VERSION").to_string(),
        accepted: summary
            .installed
            .iter()
...
/// Copy directory-style skills from seeds-cache to $HYDRA_HOME/skills/.
/// E.g., `hydra-automation-recommender/SKILL.md` + `references/`.
///
...
fn install_directory_skills_from_seeds(
    ...
    // HYDRA_HOME when set, else $HOME/.hydra.
