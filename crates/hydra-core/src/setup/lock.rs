...
//!
//! Acquire order:
//! 1. Read `.hydra/.setup.lock.sentinel` if present. If recorded PID is alive **and**
//!    its start_time matches, return [`LockError::Held`] (unless `force = true`).
//!    Stale sentinel is removed.
//! 2. `try_lock_exclusive` on `.hydra/.setup.lock` — second rail.
...
pub struct SetupLock {
    fd: File,
    /// Path to the project's `.hydra/.setup.lock` file. Kept around for
