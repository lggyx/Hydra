// Diagnostic file-trace shared with `hydra_tuix::trace`. Enabled via
// `HYDRA_TUIX_LOG=/path/to/file`. Opens in append mode so both crates
...
fn sink() -> Option<&'static Mutex<File>> {
    SINK.get_or_init(|| {
        let path = std::env::var("HYDRA_TUIX_LOG").ok()?;
        if path.is_empty() {
            return None;
        }
        // Append (not truncate) — hydra_tuix::trace opens with
