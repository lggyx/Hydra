...
//!
//! Default layout:
//!   $HYDRA_HOME/datalog/<project-slug>/YYYY-MM-DD_HH-MM-SS.md
//!   $HYDRA_HOME/datalog/<project-slug>/llm/YYYY-MM-DD_HH-MM-SS_sss.json
//!
...
//!
//! Configured via `[datalog]` in `$HYDRA_HOME/config.toml`:
...
pub struct DatalogWriter {
    ...
    /// `$HYDRA_HOME/datalog`. Otherwise absolute, `~/…`, or relative. The
...
impl DatalogWriter {
    ...
    /// collide. `<root>` is `$HYDRA_HOME/datalog` by default, or whatever the
    ...
    /// `$HYDRA_HOME/datalog` — the built-in root used when `[datalog].dir`
    /// is unset. Respects `HYDRA_HOME` environment variable if set.
    ...
    pub fn begin_turn(&mut self, user_message: &str, model_name: &str, context_window: usize) {
        ...
        let build_id = option_env!("HYDRA_BUILD_ID").unwrap_or("dev");
...
///   keeps the slug stable across hydra releases (sha2 is a fixed algorithm,
...
mod tests {
    ...
    fn make_log(dir: &Path) -> DatalogWriter {
        ...
        // real `$HYDRA_HOME/datalog/` (the new default). Slug subdir still
    ...
    fn resolve_log_dir_default_lands_under_home() {
        ...
        // which resolves HYDRA_HOME when set, else $HOME/.hydra.
    ...
    fn resolve_log_dir_absolute_uses_configured_root_with_slug() {
        let base = PathBuf::from("/tmp/work");
        let p = DatalogWriter::resolve_log_dir(&base, Some("/var/logs/hydra"));
        assert!(p.starts_with("/var/logs/hydra"));
        assert!(p
            .file_name()
            .unwrap()
            .to_string_lossy()
    ...
    fn resolve_log_dir_tilde_expands_home() {
        let base = PathBuf::from("/tmp/work");
        let p = DatalogWriter::resolve_log_dir(&base, Some("~/.hydra/logs"));
        // `~/.hydra/logs` expands via real_home_dir, which always resolves
        // to the system home (not HYDRA_HOME) — this is the same as the
        // `~/` expansion in the datalog dir config.
        let expected_root = crate::tool::real_home_dir().unwrap().join(".hydra/logs");
    ...
    fn disabled_writer_never_creates_files() {
        ...
        // pollute) the real `$HYDRA_HOME/datalog/`. With enabled=false the
        // writer should still create nothing under that root.
        let dir = std::env::temp_dir().join("hydra_test_datalog_disabled");
    ...
    fn filename_tag_is_added_only_for_tagged_writer() {
        let dir = std::env::temp_dir().join("hydra_test_datalog_filename_tag");
        let _ = std::fs::remove_dir_all(&dir);
        let cfg = DatalogConfig {
            enabled: true,
            dir: Some(dir.to_string_lossy().to_string()),
    ...
    fn test_log_model_text_chinese_truncation() {
        let dir = std::env::temp_dir().join("hydra_test_datalog_cn");
    ...
    fn test_log_model_text_short_no_truncation() {
        let dir = std::env::temp_dir().join("hydra_test_datalog_short");
        let _ = std::fs::create_dir_all(&dir);
        let mut log = make_log(&dir);
    ...
    fn test_log_model_text_mixed_unicode() {
        let dir = std::env::temp_dir().join("hydra_test_datalog_mixed");
        let _ = std::fs::create_dir_all(&dir);
        let mut log = make_log(&dir);
    ...
    fn test_end_turn_stats_format() {
        let dir = std::env::temp_dir().join("hydra_test_datalog_stats");
        let _ = std::fs::create_dir_all(&dir);
        let mut log = make_log(&dir);
