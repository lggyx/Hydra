...
use hydra_telemetry::{CurrentContext, Event as TelemetryEvent, LlmErrorKind, ToolErrorKind};
...
impl TurnRunner {
    ...
    pub async fn run_with_filter(
        ...
        let host = pcfg.and_then(|p| {
            hydra_telemetry::resolve_provider_host(&p.provider_type, p.base_url.as_deref())
        });
        ...
        // `$HYDRA_HOME/datalog/<project-slug>/`) — the same root the
        ...
        // HYDRA_FIRST_TOKEN_TIMEOUT_SECS / HYDRA_STREAM_TIMEOUT_SECS
        ...
        let first_token_timeout = timeout_from_env("HYDRA_FIRST_TOKEN_TIMEOUT_SECS", 300);
        let stream_timeout = timeout_from_env("HYDRA_STREAM_TIMEOUT_SECS", 300);
    ...
    async fn execute_single_tool(
        ...
        let output_tail = hydra_telemetry::scrub::truncate_head(
            &hydra_telemetry::scrub::scrub_path(
                &tool_result.output,
                None,
...
pub(crate) fn build_args_summary(tool_name: &str, args: &str) -> String {
                ...
                .map(|(k, v)| {
                    let val_str = match v {
                        serde_json::Value::String(s) => {
                            hydra_telemetry::scrub::truncate_head(s, 50)
                        }
    ...
    format!("{}({})", tool_name, hydra_telemetry::scrub::truncate_head(args, 100))
}
