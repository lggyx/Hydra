...
fn build_turn_system_notification_text(turn: &TurnNotification<'_>) -> (Cow<'static, str>, String) {
    let title = match turn.stop_reason {
        TurnStopReason::Natural => Cow::Borrowed("Hydra done"),
        TurnStopReason::Cancelled => Cow::Borrowed("Hydra cancelled"),
        TurnStopReason::Error => Cow::Borrowed("Hydra failed"),
        TurnStopReason::TurnLimit => Cow::Borrowed("Hydra stopped"),
        TurnStopReason::StepLimit => Cow::Borrowed("Hydra stopped"),
    };
...
fn build_approval_notification_text(
    ...
    let title = Cow::Borrowed("Hydra approval needed");
...
mod tests {
    ...
    fn terminal_text_is_compact_for_iterm() {
        let (title, body) = build_turn_terminal_notification_text(
            ...
            &TurnNotification {
                ...
                stop_reason: TurnStopReason::Natural,
                working_dir: Some(Path::new("/tmp/hydra")),
            },
        );
        assert_eq!(title, "Hydra done");
    ...
    fn approval_notification_is_action_oriented() {
        ...
        assert_eq!(title, "Hydra approval needed");
    ...
    fn background_only_is_preserved_for_approval_notifications() {
        let plan = NotificationPlan {
            title: Cow::Borrowed("Hydra approval needed"),
            body: "Bash is waiting for Y/A/N".into(),
            terminal_id: "hydra-approval",
    ...
    fn apple_terminal_has_no_native_terminal_notification_path() {
        let plan = NotificationPlan {
            title: Cow::Borrowed("Hydra done"),
            body: "Done · 12.0s".into(),
            terminal_id: "hydra-task",
            visibility: VisibilityPolicy::BackgroundOnlyBestEffort,
            emit_terminal: true,
            emit_system: true,
            emit_bell: true,
    ...
    fn approval_event_ignores_duration_threshold() {
        ...
        .unwrap();
        assert_eq!(plan.terminal_id, "hydra-approval");
