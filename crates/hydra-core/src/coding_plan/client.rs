// crates/hydra-core/src/coding_plan/client.rs
//
...
// domains). Every request carries `HYDRA_USER_AGENT` so AtomGit's
...
/// Override with the `HYDRA_CODINGPLAN_API_BASE` environment variable.
const DEFAULT_API_BASE: &str = "https://api.gitcode.com/api/v5";

/// Return the CodingPlan REST API base URL, reading
/// `HYDRA_CODINGPLAN_API_BASE` once at first call and caching the
/// result for the process lifetime.
///
/// Read order:
///   1. `HYDRA_CODINGPLAN_API_BASE` env var (trimmed, trailing `/`
...
pub fn api_base_url() -> String {
    ...
    BASE.get_or_init(|| {
        std::env::var("HYDRA_CODINGPLAN_API_BASE")
            .ok()
...
/// — run `hydra login` again" and leave the user to do it manually,
...
/// (e.g. running `hydra` against a server we never reach for
...
impl std::fmt::Display for AuthExpired {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "authentication failed ({}) — run `hydra login` again",
            self.status
        )
    }
}
...
impl Client {
    ...
    pub fn from_stored_auth() -> Result<Self> {
        if !auth::is_logged_in() {
            return Err(anyhow!(
                "not logged in — run `hydra login` (or the codingplan flow) first"
            ));
        }
...
mod tests {
    ...
    /// `anyhow!("authentication failed (NNN) — run `hydra login` again")`
    ...
    fn auth_expired_display_matches_legacy_string() {
        ...
        assert_eq!(
            e.to_string(),
            "authentication failed (401) — run `hydra login` again"
        );
        let e = AuthExpired { status: 403 };
        assert_eq!(
            e.to_string(),
            "authentication failed (403) — run `hydra login` again"
        );
    }
