// crates/hydra-core/src/coding_plan/setup.rs
//
...
///
/// Read order:
///   1. `HYDRA_CODINGPLAN_LLM_BASE_URL` env var (trimmed, trailing
...
fn codingplan_llm_base_url() -> String {
    ...
    URL.get_or_init(|| {
        std::env::var("HYDRA_CODINGPLAN_LLM_BASE_URL")
            .ok()
...
pub struct SetupReport {
    ...
    /// (TUI `/codingplan`, CLI `hydra codingplan`) read this flag
    /// to drive an inline re-OAuth + retry instead of leaving the user
    /// staring at a "claim failed — run `hydra login` again" line
...
pub fn run(
    config: &mut Config,
    tel: Option<&Arc<hydra_telemetry::Telemetry>>,
) -> Result<SetupReport> {
    // Step 1: login
    ...
    if login.is_err() {
        ...
        if let Some(t) = tel {
            t.track(hydra_telemetry::Event::TakeCodingplan {
                type_: hydra_telemetry::CodingplanResult::Fail,
                error_kind: Some(hydra_telemetry::CodingplanErrorKind::AuthError),
    ...
    if models.is_err() {
        if let Some(t) = tel {
            t.track(hydra_telemetry::Event::TakeCodingplan {
                type_: hydra_telemetry::CodingplanResult::Fail,
                error_kind: Some(hydra_telemetry::CodingplanErrorKind::NetworkError),
                error_data: Some(serde_json::json!({
                    "step": "models",
    ...
    if let Some(t) = tel {
        t.track(hydra_telemetry::Event::TakeCodingplan {
            type_: hydra_telemetry::CodingplanResult::Success,
            error_kind: None,
            error_data: Some(serde_json::json!({
                "step": null,
            }).to_string()),
...
fn step_login(tel: Option<&Arc<hydra_telemetry::Telemetry>>) -> StepResult<LoginInfo> {
...
/// of "in 23h 59m").
///
/// `pub` so the `/status` rendering in hydra-tuix can share the same
...
mod tests {
    ...
    fn codingplan_llm_base_url_defaults_to_new_signed_gateway() {
        // Lock in the default. If `HYDRA_CODINGPLAN_LLM_BASE_URL` is
        ...
        // value is fixed for the lifetime of one `hydra` run.
        let actual = codingplan_llm_base_url();
        let env_override = std::env::var("HYDRA_CODINGPLAN_LLM_BASE_URL")
            .ok()
            .map(|v| v.trim().trim_end_matches('/').to_string())
            .filter(|v| !v.is_empty());
    ...
    fn model_entry_deserialises_new_wire_shape() {
        ...
        assert_eq!(m.is_hydra_exclusive, 1);
