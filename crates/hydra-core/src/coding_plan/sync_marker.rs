// crates/hydra-core/src/coding_plan/sync_marker.rs
//
...
//
// The file lives next to `config.toml` (same `$HYDRA_HOME` / `~/.hydra`
...
mod tests {
    ...
    /// Integration-style test using a scoped HYDRA_HOME override.
    ...
    impl ScopedHome {
        fn new() -> Self {
            ...
            std::env::set_var("HYDRA_HOME", dir.path());
            Self {
                _guard: guard,
                prev,
                _dir: dir,
