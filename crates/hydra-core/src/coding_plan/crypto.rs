...
// `RealSigner`, a thin pass-through to `hydra_codingplan_crypto::sign_v1`.
...
// `crates/hydra-codingplan-crypto/` with the same API surface; the
...
/// which forwards into the closed-source `hydra-codingplan-crypto`
/// crate.
#[cfg(not(feature = "codingplan-crypto"))]
...
impl RequestSigner for RealSigner {
    ...
    fn algorithm_version(&self) -> u8 {
        hydra_codingplan_crypto::ALGORITHM_VERSION
    }
}
