...
///
/// Lowercase `hydra/<version>` is deliberate. The LLM gateway at
/// `api-ai.gitcode.com` has a UA filter that silently hijacks any
/// request whose UA starts with capital-A `Hydra` and replies
...
pub const HYDRA_USER_AGENT: &str = concat!("hydra/", env!("CARGO_PKG_VERSION"));
