...
impl Client {
    ...
    pub fn from_stored_auth() -> Result<Self> {
        if !auth::is_logged_in() {
            return Err(anyhow!("not logged in — run `hydra login` first"));
        }
        let token = auth::get_valid_token()
            .context("failed to load OAuth token (try `hydra login` again)")?;
        // Default reqwest UA (`reqwest/<ver>`) is rejected by AtomGit's gate
        // — every request here must carry `hydra/<ver>`. Builder() with
