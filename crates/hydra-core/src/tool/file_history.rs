...
//! before modification, stored in `$HYDRA_HOME/file-history/{session}/`.
...
pub struct FileHistory {
    /// Base directory: $HYDRA_HOME/file-history/{session}/
    backup_dir: PathBuf,
...
mod tests {
    ...
    async fn test_backup_and_restore() {
        let dir = std::env::temp_dir().join("hydra_test_fh_backup");
    ...
    async fn test_eviction() {
        let dir = std::env::temp_dir().join("hydra_test_fh_evict");
        let _ = std::fs::create_dir_all(&dir);
