...
/// Hydra "randomly switches working directory". The dedicated
...
mod sanitize_tests {
    ...
    fn bash_path_guard_auto_approves_workspace_relative_reads() {
        ...
        let nested = workspace.path().join("crates/hydra-core/src");
        std::fs::create_dir_all(&nested).unwrap();
        let target = nested.join("notify.rs");
        std::fs::write(&target, "pub fn notify() {}").unwrap();

        let approval =
            approval_for_command_paths("cat crates/hydra-core/src/notify.rs", workspace.path());
    ...
    fn destructive_check_allows_routine_git_ops() {
        ...
        assert!(check_destructive_command("git add crates/hydra-core/src/tool/bash.rs").is_none());
