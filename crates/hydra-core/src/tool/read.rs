...
/// model. opencode uses 50 KB; we go larger since Hydra targets
                ...
                Some(stripped) if stripped.exists() => {
                    let note = format!(
                        "[note: requested file_path ended with a `{{…}}` template fragment \
                         ...
                         hydra stripped it and read {} instead. Future read_file calls on \
...
mod tests {
    ...
    ///      mentioned (e.g. `~/.hydra/MEMORY.md`) on the next call
    ///      instead of looping.
    #[tokio::test]
