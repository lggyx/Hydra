...
//! - `$HYDRA_HOME/hooks.json` — global hooks
...
/// Load and merge hooks from global (`$HYDRA_HOME/hooks.json`) and project
...
pub fn load_hooks_config(project_dir: &Path) -> Vec<HookConfig> {
    ...
    //   2. Legacy hydra hooks.json file (fallback).
    ...
    for assets in crate::plugin::loader::iter_installed_plugin_assets() {
        if let Some(cc_map) = assets.manifest.inline_cc_hooks() {
            for (name, hook) in cc_hooks_to_hydra(cc_map, &assets.plugin_dir) {
...
/// `CLAUDE_PLUGIN_ROOT` / `HYDRA_PLUGIN_ROOT` environment variables —
...
pub(crate) fn cc_hooks_to_hydra(
    ...
    for (name, entry) in raw.hooks {
        ...
        configs.push((
            ...
            HookConfig {
                ...
                // `cc_hooks_to_hydra` which sets this.
                plugin_root: None,
            },
        ));
    }
...
mod tests {
    ...
    fn cc_hooks_to_atomcode_records_plugin_root_without_substitution() {
        ...
        let out = cc_hooks_to_hydra(&cc, plugin_root);
        assert_eq!(out.len(), 1);
        let (_, h) = &out[0];
    ...
    fn cc_hooks_to_atomcode_skips_unsupported_events() {
        ...
        assert!(cc_hooks_to_hydra(&cc, std::path::Path::new("/")).is_empty());
    }
    ...
    fn cc_hooks_to_atomcode_default_timeout_when_omitted() {
        ...
        let out = cc_hooks_to_hydra(&cc, std::path::Path::new("/"));
