...
//! mutate `HYDRA_HOME` use [`isolated_home`] to obtain an [`IsolatedHome`]
...
/// Create a fresh tempdir, point `HYDRA_HOME` at it, and return a guard
...
pub fn isolated_home() -> IsolatedHome {
    ...
    std::env::set_var("HYDRA_HOME", &path);
