...
impl GraphIndexer {
    ...
    pub async fn index_all(&mut self, cancel: CancellationToken) {
        ...
        //    at ~99% for the whole initial index, which reads as "hydra
...
///
/// The escape hatch is [`looks_like_project`]: drop a `.hydra`
...
///
/// `.hydra` is intentionally NOT in this list even though hydra
/// writes `.hydra/graph.bin` there: using hydra's own storage
/// dir as a "user opt-in" marker is self-fulfilling — the very first
/// `/cd` to any directory creates `.hydra/` and pins that dir as
...
mod tests {
    ...
    /// had already planted .hydra there.
    #[test]
    fn should_index_refuses_umbrella_with_only_atomcode_storage_dir() {
        let tmp = tempfile::TempDir::new().unwrap();
        // Simulate the state left by a prior indexer run.
        std::fs::create_dir_all(tmp.path().join(".hydra")).unwrap();
        std::fs::write(tmp.path().join(".hydra").join("graph.bin"), b"x").unwrap();
        ...
        assert!(
            !should_index(tmp.path()),
            ".hydra dir must not rescue an umbrella from the guard"
        );
    }
    ...
    async fn index_all_bails_on_cancelled_token() {
        ...
        std::fs::write(tmp.path().join(".hydra"), "").unwrap();
