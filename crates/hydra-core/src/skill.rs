...
impl SkillRegistry {
    ...
    ///
    /// Global (home dir or HYDRA_HOME):
    ///   1. `{home}/.claude/commands/*.md`          legacy flat, Claude Code compat
    ///   2. `{home}/.hydra/commands/*.md`         legacy flat, hydra native
    ///   3. `{home}/.claude/skills/*/SKILL.md`       directory-style, Claude Code compat
    ///   4. `{home}/.hydra/skills/*/SKILL.md`     directory-style, hydra native
    ///
    /// Project (working dir):
    ///   5. `.claude/commands/*.md`
    ///   6. `.hydra/commands/*.md`
    ///   7. `.claude/skills/*/SKILL.md`
    ///   8. `.hydra/skills/*/SKILL.md`
    ///
    /// Same-name skill from a `skills/` directory beats one from `commands/`
    /// at the same level because it is loaded after.
    ///
    /// Note: If HYDRA_HOME env var is set, it overrides the default home directory
    /// for hydra-specific paths (.hydra/commands and .hydra/skills).
    ...
    pub fn reload(&mut self, working_dir: &Path) -> Vec<String> {
        ...
        // Hydra config dir (respects HYDRA_HOME env var; defaults to
        // ~/.hydra). This is the SAME root used by config.toml, history,
        // plugins/, etc. — see Config::config_dir() for the single source of
        // truth.
        let hydra_config_dir = crate::config::Config::config_dir();
        ...
        // Load hydra native paths from the unified config dir.
        self.load_flat_commands(&hydra_config_dir.join("commands"), LOOSE_NS, &mut warnings);
        self.load_skills_dir(&hydra_config_dir.join("skills"), LOOSE_NS, &mut warnings);
        ...
        self.load_flat_commands(&working_dir.join(".hydra").join("commands"), LOOSE_NS, &mut warnings);
        self.load_skills_dir(&working_dir.join(".claude").join("skills"), LOOSE_NS, &mut warnings);
        self.load_skills_dir(&working_dir.join(".hydra").join("skills"), LOOSE_NS, &mut warnings);
    ...
    ///
    /// 1. **Hydra / parent-directory layout**: `dir/` contains subdirectories
    ...
    fn load_skills_dir(&mut self, dir: &Path, namespace: Option<&str>, warnings: &mut Vec<String>) {
        ...
        // Hydra / parent-directory layout: each subdirectory with a SKILL.md.
        let entries = match std::fs::read_dir(dir) {
            Ok(e) => e,
            Err(_) => return,
        };
...
mod tests {
    ...
    fn reload_picks_up_installed_plugin_skills() {
        ...
        // Under unified HYDRA_HOME semantics, plugins live at $HOME/plugins
        // (not $HOME/.hydra/plugins) — see plugin/paths.rs.
    ...
    fn reload_picks_up_cc_array_plugin_skills() {
        ...
        // Plugins live directly under HYDRA_HOME (unified semantics).
        let plugins_root = tmp.path().join("plugins");
