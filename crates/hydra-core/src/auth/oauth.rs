...
use hydra_telemetry::{Event, Telemetry};

/// Default Platform server base URL (client_secret is kept on the broker).
/// Override with the `HYDRA_PLATFORM_SERVER` environment variable.
...
/// Return the Platform server base URL, reading `HYDRA_PLATFORM_SERVER` once
...
fn platform_base_url() -> &'static str {
    ...
    BASE.get_or_init(|| {
        let raw = std::env::var("HYDRA_PLATFORM_SERVER")
            .unwrap_or_else(|_| DEFAULT_PLATFORM_SERVER.to_string());
        sanitize_base_url(&raw)
    })
}

/// Platform server URLs (derived from `HYDRA_PLATFORM_SERVER`).
...
/// Blocking HTTP client pre-configured with `HYDRA_USER_AGENT`. Every
...
/// (`hydra login`, `hydra codingplan`) and by `setup.rs`'s
...
fn accept_callback_until_stopped(
    ...
    let response = "HTTP/1.1 200 OK\r\nContent-Type: text/html; charset=utf-8\r\n\r\n\
        <html><head><title>Hydra Login</title>\
        ...
        <p>You can close this window and return to Hydra.</p>\
        </div></body></html>";
...
mod tests {
    ...
    fn parse_url_encoded_state_is_decoded() {
        let (_, state) =
            parse_pasted_callback("http://127.0.0.1:8765/callback?code=c&state=hydra_%3Atest")
                .unwrap();
        assert_eq!(state, "hydra_:test");
    }
