#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────
# Hydra npm package build script
...
fct(){ curl -sS -H "PRIVATE-TOKEN: $ATOMGIT_TOKEN" -H "Accept: application/json" \
    "$B/repos/atomgit_atomcode/hydra/contents/Cargo.toml?ref=main"; }
...
{"name":"@atomgit.com/hydra","version":"${VERSION}-${tag}","os":["${os}"],"cpu":["${arch}"],"files":["bin/"]}
EOF

  # download binary
  local dl_os="$os"
  [ "$os" = "win32" ] && dl_os="windows"
  local bin_name="hydra$([ "$os" = "win32" ] && echo ".exe")"
  local url="https://atomgit.com/atomgit_atomcode/hydra/releases/download/v${VERSION}/hydra-v${VERSION}-${dl_os}-${arch}$([ "$os" = "win32" ] && echo ".exe")"
  ...
  echo "  ✓ @atomgit.com/hydra@${VERSION}-${tag}"
}

echo ""
echo "  Publishing @atomgit.com/hydra v${VERSION}"
echo ""
...
cp "$NPM_DIR/bin/hydra.js" "$CORE_DIR/bin/"
cd "$CORE_DIR"
...
pkg.optionalDependencies = {
  '@atomgit.com/hydra-darwin-arm64': 'npm:@atomgit.com/hydra@$VERSION-darwin-arm64',
  '@atomgit.com/hydra-darwin-x64': 'npm:@atomgit.com/hydra@$VERSION-darwin-x64',
  '@atomgit.com/hydra-linux-arm64': 'npm:@atomgit.com/hydra@$VERSION-linux-arm64',
  '@atomgit.com/hydra-linux-x64': 'npm:@atomgit.com/hydra@$VERSION-linux-x64',
  '@atomgit.com/hydra-win32-x64': 'npm:@atomgit.com/hydra@$VERSION-win32-x64',
  '@atomgit.com/hydra-ohos-arm64': 'npm:@atomgit.com/hydra@$VERSION-ohos-arm64'
};
fs.writeFileSync('package.json', JSON.stringify(pkg, null, 2) + '\n');
"
npm publish --registry=https://registry.npmjs.org/ --access public $NPM_EXTRA
echo "  ✓ @atomgit.com/hydra@${VERSION} (core)"
echo ""
echo "  All done!"
