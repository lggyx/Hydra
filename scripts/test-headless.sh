#!/usr/bin/env bash
# Hydra headless-mode smoke tests
#
...
#   HYDRA_TEST_PROVIDER   Provider name to use for live (network) tests
...
TMPDIR_T=$(mktemp -d /tmp/hydra-headless-XXXXXX)
...
echo "=== Hydra Headless Smoke Tests ==="
echo "  Binary  : $BIN"
echo "  Timeout : $TIMEOUT_BIN"
echo "  TmpDir  : $TMPDIR_T"
echo "  Provider: ${HYDRA_TEST_PROVIDER:-<unset — network tests will be skipped>}"
echo ""
...
###############################################################################
if [ -z "${HYDRA_TEST_PROVIDER:-}" ]; then
    skip "T1: -p emits stdout                  (needs HYDRA_TEST_PROVIDER)"
    skip "T2: stdout has no decoration markers (needs HYDRA_TEST_PROVIDER)"
    skip "T3: -v stderr has log/diagnostic output (needs HYDRA_TEST_PROVIDER)"
    skip "T3b: default headless stderr is clean    (needs HYDRA_TEST_PROVIDER)"
    skip "T4: -p does not block on stdin       (needs HYDRA_TEST_PROVIDER)"
else
    PROV="$HYDRA_TEST_PROVIDER"
