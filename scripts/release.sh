...
# guarantees the package filename matches `hydra --version`. Git tags
# can drift (Cargo bumped but tag not yet pushed, or vice versa).
# HYDRA_VERSION env override is preserved for CI / one-off rebuilds.
...
if [ -z "$VERSION" ]; then
    ...
    echo "or override with HYDRA_VERSION=v1.0.0."
    exit 1
fi
case "$VERSION" in
    v[0-9]*) ;;
    *)
        ...
        echo "Set HYDRA_VERSION=v1.2.3 if you really mean to."
        exit 1
        ;;
esac
...
# Set HYDRA_INCLUDE_DAEMON=1 to also build and package hydra-daemon.
INCLUDE_DAEMON="${HYDRA_INCLUDE_DAEMON:-0}"
CARGO_PKG_ARGS=(-p hydra)
if [ "$INCLUDE_DAEMON" = "1" ]; then
    CARGO_PKG_ARGS+=(-p hydra-daemon)
fi
...
cp "target/${TARGET_ARM}/release/hydra" "${DIST}/hydra-${VERSION}-darwin-arm64"
echo "  -> ${DIST}/hydra-${VERSION}-darwin-arm64"
copy_daemon "target/${TARGET_ARM}/release/hydra-daemon" "${DIST}/hydra-daemon-${VERSION}-darwin-arm64" ""
...
cp "target/${TARGET_X86}/release/hydra" "${DIST}/hydra-${VERSION}-darwin-x64"
echo "  -> ${DIST}/hydra-${VERSION}-darwin-x64"
copy_daemon "target/${TARGET_X86}/release/hydra-daemon" "${DIST}/hydra-daemon-${VERSION}-darwin-x64" ""
...
if command -v x86_64-linux-musl-gcc &>/dev/null; then
    ...
    cp "target/${TARGET_LINUX}/release/hydra" "${DIST}/hydra-${VERSION}-linux-x64"
    echo "  -> ${DIST}/hydra-${VERSION}-linux-x64"
    copy_daemon "target/${TARGET_LINUX}/release/hydra-daemon" "${DIST}/hydra-daemon-${VERSION}-linux-x64" ""
else
...
if command -v aarch64-linux-musl-gcc &>/dev/null; then
    ...
    cp "target/${TARGET_LINUX_ARM}/release/hydra" "${DIST}/hydra-${VERSION}-linux-arm64"
    echo "  -> ${DIST}/hydra-${VERSION}-linux-arm64"
    copy_daemon "target/${TARGET_LINUX_ARM}/release/hydra-daemon" "${DIST}/hydra-daemon-${VERSION}-linux-arm64" ""
else
...
if command -v x86_64-w64-mingw32-gcc &>/dev/null; then
    ...
    cp "target/${TARGET_WIN}/release/hydra.exe" "${DIST}/hydra-${VERSION}-windows-x64.exe"
    echo "  -> ${DIST}/hydra-${VERSION}-windows-x64.exe"
    copy_daemon "target/${TARGET_WIN}/release/hydra-daemon" "${DIST}/hydra-daemon-${VERSION}-windows-x64" ".exe"
else
...
if command -v aarch64-w64-mingw32-gcc &>/dev/null; then
    ...
    cp "target/${TARGET_WIN_ARM}/release/hydra.exe" "${DIST}/hydra-${VERSION}-windows-arm64.exe"
    echo "  -> ${DIST}/hydra-${VERSION}-windows-arm64.exe"
    copy_daemon "target/${TARGET_WIN_ARM}/release/hydra-daemon" "${DIST}/hydra-daemon-${VERSION}-windows-arm64" ".exe"
else
    echo "  !! Skipped: llvm-mingw not installed (brew install llvm-mingw or see https://github.com/mstorsjo/llvm-mingw)"
fi

# --- Sign macOS hydra binaries (skip with HYDRA_SKIP_SIGN=1) ---
if [ "${HYDRA_SKIP_SIGN:-0}" != "1" ]; then
    echo ""
    echo "=== Signing macOS hydra binaries ==="
    "$(dirname "$0")/sign-macos.sh" "$DIST"
else
    echo ""
    echo "=== Skipping macOS signing (HYDRA_SKIP_SIGN=1) ==="
fi
...
echo "=== SHA256 ==="
cd "$DIST"
shasum -a 256 hydra-* 2>/dev/null | tee checksums.txt
...

{
    ...
    first=1
    for pair in \
        "darwin-arm64:hydra-${VERSION}-darwin-arm64" \
        "darwin-x64:hydra-${VERSION}-darwin-x64" \
        "linux-x64:hydra-${VERSION}-linux-x64" \
        "linux-arm64:hydra-${VERSION}-linux-arm64" \
        "ohos-arm64:hydra-${VERSION}-ohos-arm64" \
        "windows-x64:hydra-${VERSION}-windows-x64.exe" \
        "windows-arm64:hydra-${VERSION}-windows-arm64.exe"
    do
        target="${pair%%:*}"
        file="${pair#*:}"
        if [ -f "$file" ]; then
