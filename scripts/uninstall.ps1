...
#   -Purge            delete everything including %USERPROFILE%\.hydra
...
# locate install dir
$InstallDir = if ($env:HYDRA_PREFIX) { $env:HYDRA_PREFIX } else { Join-Path $env:LOCALAPPDATA "Hydra" }
$Binary = Join-Path $InstallDir "hydra.exe"

$DataDir = if ($env:HYDRA_HOME) { $env:HYDRA_HOME } else { Join-Path $env:USERPROFILE ".hydra" }

# plan
Write-Host "Will remove (Group 1):"
if (Test-Path $Binary) { Write-Host "  $Binary" }
foreach ($f in @("hydra.exe.bak",".hydra.rolling",".hydra.download",".hydra.writable-probe")) {
...
if ($cur) {
    ...
    $kept = $entries | Where-Object {
        ...
        $_.TrimEnd('\').ToLower() -ne ($env:LOCALAPPDATA + '\Hydra').ToLower()
    }
...
if (Test-Path $Binary) {
    try {
        ...
        Write-Error "could not remove $Binary — close any running hydra.exe and re-run."
        exit 4
    }
}
