...
step "Building hydra (debug)"
cd "$PROJECT_ROOT"
cargo build 2>&1 | tail -5
success "Debug build OK"

if "$DO_RELEASE"; then
    step "Building hydra (release)"
    cargo build --release 2>&1 | tail -5
    success "Release build OK: target/release/hydra"
fi
...
cd "$PROJECT_ROOT"
echo ""
echo -e "${C_GREEN}${C_BOLD}Hydra dev environment ready.${C_RESET}"
echo ""
echo "  Quick start:"
