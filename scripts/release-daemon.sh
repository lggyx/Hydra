...
# Build hydra-daemon artifacts used by the VS Code extension package.
...
if [ -z "$VERSION" ]; then
    ...
    echo "or override with HYDRA_VERSION=v1.2.3."
    exit 1
fi
case "$VERSION" in
    v[0-9]*) ;;
...
build_daemon() {
    local target="$1"
    local suffix="$2"
    local exe="${3:-hydra-daemon}"
    local env_prefix="${4:-}"

    echo "Building ${target}..."
    rustup target add "$target" >/dev/null 2>&1 || true
    if [ -n "$env_prefix" ]; then
        eval "$env_prefix cargo build --release --target \"$target\" -p hydra-daemon"
    else
        cargo build --release --target "$target" -p hydra-daemon
    fi

    local src="target/${target}/release/${exe}"
    local dst="${DIST}/hydra-daemon-${VERSION}-${suffix}"
    if [[ "$exe" == *.exe ]]; then
        dst="${dst}.exe"
    fi
...
echo "=== Hydra Daemon Release ${VERSION} ==="
echo ""
...
build_daemon "x86_64-pc-windows-gnu" "windows-x64" "hydra-daemon.exe"

echo ""
echo "=== SHA256 ==="
(
    cd "$DIST"
    shasum -a 256 hydra-daemon-* | tee daemon-checksums.txt
)

echo ""
echo "=== VS Code packaging env ==="
cat <<EOF
HYDRA_DAEMON_DARWIN_ARM64="${ROOT}/${DIST}/hydra-daemon-${VERSION}-darwin-arm64" \\
HYDRA_DAEMON_DARWIN_X64="${ROOT}/${DIST}/hydra-daemon-${VERSION}-darwin-x64" \\
HYDRA_DAEMON_LINUX_X64="${ROOT}/${DIST}/hydra-daemon-${VERSION}-linux-x64" \\
HYDRA_DAEMON_LINUX_ARM64="${ROOT}/${DIST}/hydra-daemon-${VERSION}-linux-arm64" \\
HYDRA_DAEMON_WIN32_X64="${ROOT}/${DIST}/hydra-daemon-${VERSION}-windows-x64.exe" \\
npm --prefix "${ROOT}/extensions/vscode" run package
EOF

echo ""
echo "Done. Daemon artifacts in ${DIST}/"
ls -lh "${DIST}"/hydra-daemon-*
