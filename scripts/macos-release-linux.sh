#!/usr/bin/env bash
# Linux cross-build script for Hydra daemon.
#
# Produces Linux release artifacts for hydra-daemon, cross-compiled from macOS.
#
...
echo "=== Hydra Linux Release ${VERSION} (cross-compile from macOS) ==="
echo ""
...
echo ""
echo "请选择构建范围："
echo "  1) 仅 hydra-daemon"
echo "  2) hydra-daemon + hydra CLI"
echo ""
...
echo "产物:     hydra-daemon$([ "$INCLUDE_CLI" = "1" ] && echo " + hydra CLI")"
echo "输出目录: ${DIST}"
echo ""
...
copy_cli() {
    ...
    local target="$1"
    local suffix="$2"
    local src="target/${target}/release/hydra"
    local dst="${DIST}/hydra-${VERSION}-${suffix}"
    cp "$src" "$dst"
    echo "  -> $dst"
}
...
build_linux_x64() {
    ...
    local out="${DIST}/hydra-daemon-${VERSION}-${suffix}"
    cp "target/${target}/release/hydra-daemon" "$out"
    echo "  -> $out"
...
echo "=== SHA256 ==="
cd "$DIST"
shasum -a 256 hydra-*linux-* 2>/dev/null | tee checksums-linux.txt

echo ""
echo "Done. Linux artifacts:"
ls -lh hydra-*linux-* checksums-linux.txt 2>/dev/null
