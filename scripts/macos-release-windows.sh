#!/usr/bin/env bash
# Windows cross-build script for Hydra.
#
...
#
# Environment:
#   HYDRA_VERSION=vX.Y.Z       Override version. Defaults to Cargo.toml.
#   HYDRA_WINDOWS_TARGETS=x64  Comma-separated: x64,arm64,all. Defaults to x64.
#   HYDRA_INCLUDE_DAEMON=1     Also build hydra-daemon.exe.
#   HYDRA_USE_MIRROR=1         Use rsproxy.cn (ByteDance) mirror for faster
...
# When HYDRA_USE_MIRROR=1, set rustup + cargo to use the fast domestic mirror.
# This dramatically speeds up crate downloads and rustup target installs.
if [ "${HYDRA_USE_MIRROR:-0}" = "1" ]; then
    export RUSTUP_DIST_SERVER="https://rsproxy.cn"
    export RUSTUP_UPDATE_ROOT="https://rsproxy.cn/rustup"

    CARGO_CONFIG="$HOME/.cargo/config.toml"
    MIRROR_MARKER="# hydra-mirror-rsproxy"
...
if [ -z "$VERSION" ]; then
    echo "Could not determine version. Set HYDRA_VERSION=v1.2.3."
    exit 1
fi
...
want_target() {
    local name="$1"
    local requested="${HYDRA_WINDOWS_TARGETS:-x64}"
...
copy_daemon() {
    ...
    local target="$1"
    local suffix="$2"
    local src="target/${target}/release/hydra-daemon.exe"
    local dst="${DIST}/hydra-daemon-${VERSION}-${suffix}.exe"
    cp "$src" "$dst"
    echo "  -> $dst"
}
...
build_windows_x64() {
    ...
    local out="${DIST}/hydra-${VERSION}-${suffix}.exe"
    cp "target/${target}/release/hydra.exe" "$out"
    echo "  -> $out"
...
echo "=== Hydra Windows Release ${VERSION} ==="
echo "Artifacts: ${DIST}"
echo "Targets: ${HYDRA_WINDOWS_TARGETS:-x64}"
echo ""
...
echo "=== SHA256 ==="
cd "$DIST"
shasum -a 256 hydra-*windows-*.exe | tee checksums-windows.txt

echo ""
echo "Done. Windows artifacts:"
ls -lh hydra-*windows-*.exe checksums-windows.txt
